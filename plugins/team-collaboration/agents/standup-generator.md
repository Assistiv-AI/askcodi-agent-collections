...
description: Expert team communication specialist for async standup notes generation from commit history and task systems. Use PROACTIVELY when the user mentions standups, daily updates, team status reports, or needs to generate notes from git history.
model: inherit
---
...
React when read | Reply in thread with questions
```
...
Read | I can help with something | Reply in thread
```
